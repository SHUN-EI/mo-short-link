...
import com.mo.enums.ClientTypeEnum;
...
public class AlipayStrategy implements PayStrategy {
    ...
    public String unifiedOrder(PayInfoVO payInfoVO) {
        ...
        payParam.put("out_trade_no", payInfoVO.getOutTradeNo());
        payParam.put("product_code", "FAST_INSTANT_TRADE_PAY");
        //订单总金额，单位为元，精确到小数点后两位
        payParam.put("total_amount", payInfoVO.getPayFee().toString());
        //商品标题/交易标题/订单标题/订单关键字等。 注意：不可使用特殊字符，如 /，=，&amp; 等。
        payParam.put("subject", payInfoVO.getTitle());
        //商品描述，可空
        payParam.put("body", payInfoVO.getDescription());
        ...
        payParam.put("timeout_express", Double.valueOf(timeout).intValue() + "m");
        ...
        try {
            if (ClientTypeEnum.H5.name().equalsIgnoreCase(clientType)){
                //H5手机网页支付 AlipayTradeWapPayRequest
                AlipayTradeWapPayRequest request = new AlipayTradeWapPayRequest();
                request.setBizContent(JSON.toJSONString(payParam));
                ...
                //PC支付 AlipayTradePagePayRequest
